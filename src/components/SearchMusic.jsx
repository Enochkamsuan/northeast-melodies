    ...
    <section id="explore" className="mx-auto max-w-7xl px-6 py-20">
      <div className="flex flex-col items-start justify-between gap-3 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 max-w-xl text-muted-foreground">Filter by dialect, genre or mood to find your next favorite track.</p>
        </div>
        <span className="rounded-full bg-secondary px-3 py-1 text-xs text-muted-foreground">{results.length} results</span>
      </div>
      ...
      <div className="mt-8 glass rounded-2xl p-4 sm:p-6">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Select label="Dialect / Language" value={dialect} onChange={setDialect} options={DIALECTS} />
      ...
      <div className="mt-10 grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
        {results.map((s, i) => <SongCard key={s.id} song={s} index={i} />)}
      </div>
